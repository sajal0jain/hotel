...
const DEMO_ACCOUNTS = [
  ...
  },
  {
    ...
    description: '70-Room matrix • Live WhatsApp concierge • Urgent escalation resolution • Check-in/out'
  }
];
...
export default function LoginModal({ isOpen, onClose, onLoginSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
      <div className="relative glass-card max-w-lg w-full p-6 sm:p-8 space-y-6 border border-teal-500/30 shadow-2xl rounded-2xl bg-slate-900/95 overflow-hidden">
        ...
        <div className="relative flex items-start justify-between border-b border-slate-800/80 pb-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center shadow-lg shadow-teal-500/25 text-slate-950 font-bold">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Remedra Hotels and Residences (70 Rooms)</p>
            </div>
          </div>
          <button
            onClick={onClose}
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-xs text-slate-100 group-hover:text-teal-300 transition-colors">
                          {acc.name}
                        </span>
                  ...
                  <div className="shrink-0 flex items-center gap-1 mt-1">
                    <span className="text-[10px] px-2 py-0.5 rounded-lg bg-teal-500/20 text-teal-300 font-bold group-hover:bg-teal-500 group-hover:text-slate-950 transition-all flex items-center gap-1">
                      <span>Login</span>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleLogin(); }} className="space-y-3.5 pt-3 border-t border-slate-800/80">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 focus-within:border-teal-500/50 focus-within:ring-1 focus-within:ring-teal-500/30 gap-2.5 transition-all">
          ...
          <div className="pt-1 flex items-center gap-3">
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-400 hover:to-teal-500 text-slate-950 font-bold text-xs shadow-lg shadow-teal-500/20 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
